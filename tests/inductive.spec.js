...
test('inductive analysis without errors', async ({ page, context }) => {
  ...
  expect(
    newPage.getByRole('heading', { name: 'Data exploration¶' }).isVisible()
  )
  await newPage.getByPlaceholder('Preconception 1', { exact: true }).click()
  await newPage
    .getByPlaceholder('Preconception 1', { exact: true })
    .fill('Test preconception')
  ...
  expect(
    newPage.getByRole('heading', { name: 'Observation 1: Tests¶' }).isVisible()
  )
  await newPage.getByRole('button', { name: 'Ready to summarize' }).click()
  await newPage.getByPlaceholder('Summary', { exact: true }).click()
  await newPage
    .getByPlaceholder('Summary', { exact: true })
    .fill('Test summary')
  expect(
    newPage.getByRole('heading', { name: 'Summary: Test summary¶' }).isVisible()
  )
})
...
test('inductive analysis with errors', async ({ page, context }) => {
  ...
  await newPage.getByRole('button', { name: 'Save preconceptions' }).click()
  expect(
    newPage.getByText('You must name at least one preconception').isVisible()
  )
  await newPage.getByPlaceholder('Preconception 1', { exact: true }).click()
  await newPage
    .getByPlaceholder('Preconception 1', { exact: true })
...
test('inductive analysis all done shows export buttons', async ({
  page,
  context,
}) => {
  // @ts-ignore
