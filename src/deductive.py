...
class Deductive:
    ...
    def __init__(self, start_cell):
        ...
        self.cell_count = 1
        self.utils = BOUtils()
        self.hypothesis_input = self.utils.create_input_field()
        self.empty_hypo_error = self.utils.create_error_message()
        self.null_input = self.utils.create_input_field()
        self.empty_null_error = self.utils.create_error_message()
    ...
    def create_hypotheses_grid(self):
        """Creates widgets"""
        hypothesis_label = self.utils.create_label('Hypothesis:')
        null_label = self.utils.create_label('Null hypothesis:')
        save_button = self.utils.create_button(
            desc='Save',
            command=self.save_hypotheses)
        clear_button = self.utils.create_button(
            desc='Clear',
            command=self.clear_hypotheses,
            style='primary')
        empty = self.utils.create_placeholder()

        grid = self.utils.create_grid(
            5,
            2,
            [empty,
    ...
    def save_hypotheses(self, _=None):
        ...
        if self.check_hypotheses():
            confirmed_hypothesis = self.utils.create_message(
                value=f'Hypothesis: {self.hypothesis_input.value}')
            confirmed_null = self.utils.create_message(
    ...
    def create_run_button(self, hypothesis, null_hypothesis):
        ...
        def run_cells(_=None):
            ...
            conclusion_label = self.utils.create_message(
                value='Accepted hypothesis:')
            conclusion = self.utils.create_radiobuttons(
